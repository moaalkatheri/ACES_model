...
def evaluate(param_names, param_values, k):
    ...
    for j in range(0, m):
        ...
        if len(param_names[j]) == 4:
            ...
            text = "UPDATE "+"'"+table+"' SET "+"'"+update_var+"'=? WHERE " + \
                "'"+col_names[0]+"'=? and "+"'"+col_names[1]+"'=?"
            text = text.replace("'", "")
            ...
            text = "UPDATE "+"'"+table+"' SET "+"'"+update_var+"'=? WHERE "+"'" + \
                col_names[0]+"'=? and "+"'"+col_names[1]+"'=? and "+"'"+col_names[2]+"'=?"
            text = text.replace("'", "")
            ...
            text = "UPDATE "+"'"+table+"' SET "+"'"+update_var+"'=? WHERE "+"'" + \
                col_names[0]+"'=? and "+"'"+col_names[1]+"'=? and " + \
                "'"+col_names[2]+"'=? and "+"'"+col_names[3]+"'=?"
            text = text.replace("'", "")
    ...
    data[10] = '--input=data_files/Method_of_Morris'+str(k)+'.db'
    # 14th line in the config file referring to output database
    data[14] = '--output=data_files/Method_of_Morris'+str(k)+'.db'
...
for i in range(0, len(output_query)):
    param_names[i] = ['CostVariable', output_query[i][0],
                      output_query[i][1], output_query[i][2], 'cost_variable']
...
g3 = len(output_query)
for i in range(0, len(output_query)):
    param_names[i+g1+g2] = ['Efficiency', output_query[i][0], output_query[i]
                            [1], output_query[i][2], output_query[i][3], 'efficiency']
    with open(os.getcwd()+'/Method_of_Morris.txt', 'a') as file:
...
Morris_Objectives = Parallel(n_jobs=num_cores)(delayed(evaluate)(
    param_names, param_values[i, :], i) for i in range(0, n))
