...
class OutputPlotGenerator:
    ...
    def __init__(self, path_to_db, region, scenario, super_categories=False):
        ...
        self.folder_name = os.path.splitext(os.path.basename(path_to_db))[
            0] + "_" + region + "_" + scenario + "_plots"
    ...
    def extractFromDatabase(self, type):
        ...
        cur = con.cursor()
        if (type == 1):
            cur.execute("SELECT sector, t_periods, tech, capacity FROM Output_CapacityByPeriodAndTech WHERE scenario == '" +
                        self.scenario+"' AND regions LIKE '"+self.region+"'")
            self.capacity_output = cur.fetchall()
            self.capacity_output = [list(elem) for elem in self.capacity_output]
        elif (type == 2):
            cur.execute("SELECT sector, t_periods, tech, SUM(vflow_out) FROM Output_VFlow_Out WHERE scenario == '" +
                        self.scenario+"' AND regions LIKE '"+self.region+"' GROUP BY sector, t_periods, tech")
            self.output_vflow = cur.fetchall()
            self.output_vflow = [list(elem) for elem in self.output_vflow]
        elif (type == 3):
            cur.execute("SELECT sector, t_periods, emissions_comm, SUM(emissions) FROM Output_Emissions WHERE scenario == '" +
                        self.scenario+"' AND regions LIKE '"+self.region+"' GROUP BY sector, t_periods, emissions_comm")
    ...
    def makeStackedBarPlot(self, data, xlabel, ylabel, xvar, title):
        ...
        plt.title(title)
        lgd = plt.legend([h[0] for h in handles], stackedBars,
                         bbox_to_anchor=(1.2, 1), fontsize=7.5)
        # plt.show()
...
def GeneratePlot(args):
    ...
    parser.add_argument('-r', '--region', action="store", dest="region",
                        help="Region name, input 'global' if global results are desired", required=True)
    parser.add_argument('-s', '--scenario', action="store", dest="scenario",
                        help="Model run scenario name", required=True)
    ...
    parser.add_argument('-c', '--sector', action="store", dest="sector",
                        help="Sector for which plot to be generated", required=True)
    parser.add_argument('-o', '--output', action="store", dest="output_dir",
                        help='Output plot location', default='./')
    parser.add_argument('--super', action="store_true", dest="super_categories",
                        help="Merge Technologies or not", default=False)

    options = parser.parse_args(args)

    result = OutputPlotGenerator(options.input, options.region,
                                 options.scenario, options.super_categories)
    error = ''
    ...
    if (error == ''):
        ...
        print("Done. Look for output plot images in directory:" +
